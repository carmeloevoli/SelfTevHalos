...
inline double gamma2(double p) {
  return 1. + utils::pow_integer<2>(p / cgs::me_c);
}

inline double beta(double p) {
  return std::sqrt(1. - 1. / gamma2(p));
}

inline double larmor_radius(double p, double B) {
  return p * cgs::c_light / cgs::elementary_charge / B;
}

inline double resonant_momentum(double k, double B) {
  return cgs::elementary_charge * B / cgs::c_light / k;
}
...
inline double source_evolution(double t, double t_decay) {
