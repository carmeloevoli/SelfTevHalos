...
void Params::print() {
	...
	parfile << "\n";
	parfile << "magnetic_energy_density : " << pow2(magnetic_field.get()) / 2. / vacuum_permeability / (erg / cm3) << " erg/cm3\n";
	parfile << "vA_infty : " << magnetic_field.get() / std::sqrt(vacuum_permeability * proton_mass * ion_number_density.get()) / (km / s) << " km/s\n";
	parfile << "k0 : " << 1. / correlation_length.get() * parsec << " pc-1\n";
	parfile.close();
	std::cout << "done!\n";
}
